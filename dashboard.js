  ...
  afterDatasetsDraw(chart, args, options) {
    ...
    chart.data.datasets.forEach((dataset, datasetIndex) => {
      ...
      meta.data.forEach((bar, index) => {
        ...
        const unit = Array.isArray(options.units) ? options.units[datasetIndex] : options.unit;
        const label = `${formatted(value)} ${unit || ""}`.trim();
        if (options.orientation === "vertical") {
          const labelLength = ctx.measureText(label).width;
          const barHeight = Math.abs(bar.base - bar.y);
          const placeInside = options.position === "inside" || (options.position === "auto" && barHeight >= labelLength + 18);
          ctx.save();
          ctx.fillStyle = placeInside ? (options.insideColor || "#fff") : (options.color || "#18334e");
          ctx.textBaseline = "middle";
          ctx.textAlign = placeInside ? "right" : "left";
          ctx.translate(bar.x, placeInside ? bar.y + 8 : bar.y - 6);
          ctx.rotate(-Math.PI / 2);
          ctx.fillText(label, 0, 0);
          ctx.restore();
        } else {
          ctx.fillStyle = options.color || "#18334e";
          ctx.textAlign = "center";
          ctx.textBaseline = "bottom";
          ctx.fillText(label, bar.x, Math.max(chartArea.top + 13, bar.y - 7));
        }
      });
    });
    ctx.restore();
...
async function load() {
  ...
  try {
    const results = await Promise.allSettled([
      ...
      request("catalogos"), request("consumo_insumos", common)
    ]);
    ...
    renderInputs(data[15],data[6],results[15].status==='fulfilled',results[6].status==='fulfilled');
...
function renderProduction(rows) {
  ...
  const labels=Object.keys(weeks).sort(); chart("weeklyChart","bar",labels,[{label:"Metros lineales (m)",data:labels.map(x=>weeks[x].ml),backgroundColor:colors.blue,yAxisID:"y"},{label:"Volumen (m³)",data:labels.map(x=>weeks[x].m3),backgroundColor:colors.green,yAxisID:"y1"}],false,{layout:{padding:{top:70}},plugins:{legend:{position:"bottom"},valueLabels:{enabled:true,units:["m","m³"],fontSize:10,orientation:"vertical",position:"auto",insideColor:"#fff"}},scales:{y:{beginAtZero:true,grace:"18%",title:{display:true,text:"m"}},y1:{beginAtZero:true,grace:"18%",position:"right",grid:{drawOnChartArea:false},title:{display:true,text:"m³"}}}});
...
function renderInputs(inputs, shipments, inputsOk, shipmentsOk) {
  const grouped = {};
  (inputs || []).forEach(row => {
    const key = normalized(row.insumo);
    (grouped[key] ||= {cantidad:0,registros:0}).cantidad += n(row.cantidad);
    grouped[key].registros++;
  });
  const amount = key => grouped[key]?.registros ? grouped[key].cantidad : null;
  const diesel = amount("DIESEL"), explosive = amount("EXPLOSIVO");
  const tons = shipmentsOk ? sum(shipments || [],"tonelaje") : null;
  $('kpiBrocas').textContent=inputsOk&&amount("BROCA")!==null?formatted(amount("BROCA"),0):"—";
  $('kpiBarras').textContent=inputsOk&&amount("BARRA")!==null?formatted(amount("BARRA"),0):"—";
  $('kpiDieselTotal').textContent=inputsOk&&diesel!==null?`${formatted(diesel,1)} L`:"—";
  $('kpiExplosiveTotal').textContent=inputsOk&&explosive!==null?`${formatted(explosive,1)} kg`:"—";
  $('kpiShipmentForInputs').textContent=tons!==null?`${formatted(tons,1)} t`:"—";
  $('kpiDieselRate').textContent=inputsOk&&diesel!==null&&tons>0?`${formatted(diesel/tons,2)} L/t`:"—";
  $('kpiExplosiveRate').textContent=inputsOk&&explosive!==null&&tons>0?`${formatted(explosive/tons,3)} kg/t`:"—";
  const messages=[];
  if(!inputsOk)messages.push("No se pudieron consultar los consumos de almacén.");
  else if(!inputs.length)messages.push("Sin consumos registrados en el periodo.");
  if(!shipmentsOk)messages.push("No se pudieron consultar los embarques a planta.");
  else if(tons<=0)messages.push("No hay toneladas enviadas a planta; no se calcula una razón por tonelada.");
  messages.push("Consumos de almacén y embarques acumulados de la misma mina y el mismo rango de fechas. Las piezas se muestran solo cuando hay registros.");
  $('inputsNote').textContent=messages.join(" ");
}
...
function compareBars(id,planned,actual,unit) {
  chart(id,"bar",["Plan proporcional","Real acumulado"],[{label:unit,data:[planned,actual],backgroundColor:[colors.blue,colors.gold],borderRadius:6,borderSkipped:false,maxBarThickness:72}],false,{layout:{padding:{top:24}},plugins:{legend:{display:false},valueLabels:{enabled:true,unit},tooltip:{callbacks:{label:ctx=>ctx.parsed.y===null?"Sin datos":`${formatted(ctx.parsed.y)} ${unit}`}}},scales:{x:{grid:{display:false}},y:{beginAtZero:true,grace:"18%",title:{display:true,text:unit}}}});
}
...
document.querySelectorAll(".area-detail").forEach(detail=>detail.addEventListener("toggle",()=>{if(detail.open){document.querySelectorAll(".area-detail").forEach(other=>{if(other!==detail)other.open=false;});requestAnimationFrame(()=>Object.values(charts).forEach(instance=>instance.resize()));}else{detail.querySelectorAll(".records-detail").forEach(records=>records.open=false);}}));
