...
let catalogRows = [];
const state = { work: [], movement: [], equipment: {}, mpReasons: [], complianceCauses: [], plans: [], plant: [] };
...
function populateMineCatalogs() {
  ...
  const diesel = rows("EQUIPO", mine).filter(x => ["SCOOPTRAM", "CAMION_BAJO_PERFIL"].includes(x.grupo));
  ...
  const stope = rows("EQUIPO", mine, "STOPE_MATE");
  $('stopeSection').classList.toggle("hidden", !stope.length);
  $('stopeEquipment').innerHTML = options(stope);
  $('mpReason').innerHTML = options(rows("MOTIVO_NO_OPERACION", mine, "MAQUINA_PIERNA"));
  $('complianceReason').innerHTML = options(rows("MOTIVO_NO_OPERACION", mine, "MAQUINA_PIERNA"));
  renderEquipmentRows();
  loadDailyPlan();
}
...
function populateExploration() {
  ...
  $('explorationEquipment').innerHTML = options(rows("EQUIPO", mine, "TERMITA"));
...
function updateWorkType() {
  ...
  detail.innerHTML = isCuele ? options(rows("TIPO_TRABAJO", "", "CUELE"), "Clasificar cuele") : '<option value="">No aplica</option>';
...
function previewMovement() { const c = movementCalculation(); $('movementEstimate').textContent = c.volume > 0 && c.density > 0 ? `${c.volume.toFixed(2)} m³ · ${c.tons.toFixed(2)} t` : "—"; }
function addMovement() {
  ...
  state.movement.push({ id: uid(), equipo: equipment, familiaEquipo: eq.grupo, tipoConteo: countType, cantidad: count, procedencia: origin, destino: destination, material: materialName, capacidadYd3: c.capacity, densidadTM3: c.density, factorLlenado: c.fill, volumenM3: c.volume, tonelajeEstimado: c.tons });
...
function reasonOptions(group, selected = "") { return options(rows("MOTIVO_NO_OPERACION", $('mina').value, group)).replace(`value="${esc(selected)}"`, `value="${esc(selected)}" selected`); }
function failureOptions(selected = "") { return options(rows("TIPO_FALLA", $('mina').value, "EQUIPOS_DIESEL")).replace(`value="${esc(selected)}"`, `value="${esc(selected)}" selected`); }
function renderEquipmentRows() {
  const mine = $('mina').value, equipments = rows("EQUIPO", mine).filter(x => ["SCOOPTRAM", "CAMION_BAJO_PERFIL", "STOPE_MATE"].includes(x.grupo));
  ...
  $('equipmentRows').innerHTML = equipments.map((eq, i) => {
    const saved = state.equipment[eq.nombre] || { familiaEquipo: eq.grupo, estadoTecnico: "", horasOperadas: "", motivoNoOperacion: "", tipoFalla: "", comentarios: "" };
    state.equipment[eq.nombre] = saved;
    return `<div class="equipment-row equipment-v17-grid" data-eq-row="${esc(eq.nombre)}"><strong>${esc(eq.nombre)}</strong><select data-eq-field="estadoTecnico"><option value="">Seleccionar</option><option ${saved.estadoTecnico === "Disponible" ? "selected" : ""}>Disponible</option><option ${saved.estadoTecnico === "No disponible" ? "selected" : ""}>No disponible</option></select><input data-eq-field="horasOperadas" type="number" min="0" max="24" step="0.1" value="${esc(saved.horasOperadas)}" placeholder="0–24" /><select data-eq-field="motivoNoOperacion" ${saved.estadoTecnico === "No disponible" ? "" : "disabled"}>${reasonOptions("EQUIPOS_DIESEL", saved.motivoNoOperacion)}</select><select data-eq-field="tipoFalla" ${saved.motivoNoOperacion === "Falla de Equipo" ? "" : "disabled"}>${failureOptions(saved.tipoFalla)}</select><input data-eq-field="comentarios" value="${esc(saved.comentarios)}" placeholder="Observación" /></div>`;
  ...
  document.querySelectorAll('[data-eq-row]').forEach(row => row.querySelectorAll('[data-eq-field]').forEach(input => input.addEventListener("change", () => {
    ...
    if (field === "motivoNoOperacion" && input.value !== "Falla de Equipo") item.tipoFalla = "";
    if (["estadoTecnico", "motivoNoOperacion"].includes(field)) renderEquipmentRows();
  })));
}

function addMpReason() {
  const motivo = $('mpReason').value, cantidadAfectada = number($('mpAffected').value), comentarios = $('mpReasonComment').value.trim();
  if (!motivo || cantidadAfectada < 1) return alert("Selecciona motivo y cantidad afectada.");
  state.mpReasons.push({ id: uid(), motivo, cantidadAfectada, comentarios }); $('mpReason').value = ""; $('mpAffected').value = ""; $('mpReasonComment').value = ""; renderMpReasons();
}
function renderMpReasons() {
  $('mpReasonRows').innerHTML = state.mpReasons.length ? state.mpReasons.map(x => `<div class="data-row mp-grid"><span>${esc(x.motivo)}</span><span>${x.cantidadAfectada}</span><span>${esc(x.comentarios || "—")}</span><button class="delete-button" type="button" data-delete-mp="${x.id}">×</button></div>`).join("") : '<div class="empty-state">Sin causas registradas.</div>';
  document.querySelectorAll('[data-delete-mp]').forEach(b => b.onclick = () => { state.mpReasons = state.mpReasons.filter(x => x.id !== b.dataset.deleteMp); renderMpReasons(); });
}
...
async function loadDailyPlan() {
  ...
  if (!mine || !date) return updateSelectedPlan();
  try {
...
function validateCounts() { const values = ['mpTotal','mpAvailable','mpScheduled','mpOperating'].map(id => number($(id).value)); if (values[1] > values[0] || values[2] > values[0] || values[3] > values[2]) { alert("Revisa máquinas de pierna: disponibles/programadas no pueden superar el total y operando no puede superar programadas."); return false; } return true; }
