...
$('fecha').value = today; $('explorationDate').value = today; $('planningDate').value = today;
...
async function loadPlanningActivities() {
  ...
  try {
    ...
    const operationByPlace = summarizeOperationsByPlace(operationResult.datos || []);
    const plannedKeys = new Set((planResult.datos || []).map(plan => normalizeKey(plan.lugar)));
    state.planningItems = (planResult.datos || []).map(plan => ({
      ...plan,
      operacion: operationByPlace.get(normalizeKey(plan.lugar)) || null,
      pueblesRealizados: "",
      disparosRealizados: "",
      comentariosReal: "",
      causas: []
    }));
    state.planningUnmatched = [...operationByPlace.entries()].filter(([key]) => !plannedKeys.has(key)).map(([, summary]) => summary);
...
function summarizeOperationsByPlace(items) {
  const summaries = new Map();
  items.forEach(item => {
    const key = normalizeKey(item.obra);
    if (!key) return;
    if (!summaries.has(key)) summaries.set(key, { lugar: item.obra, metrosLineales: 0, m3: 0, tipos: new Set(), materiales: new Set(), reportes: new Set() });
...
function operationSummaryMarkup(summary) {
  if (!summary) return '<div class="planning-operation empty"><strong>Reporte de Mina</strong><span>Sin producción reportada para este lugar y fecha.</span></div>';
  const measures = [];
  ...
  return `<div class="planning-operation"><strong>Reporte de Mina</strong><span>${esc(measures.join(" · ") || "Sin cantidad reportada")}</span><small>${esc([...summary.tipos].join(", ") || "Sin tipo de trabajo")} · ${esc([...summary.materiales].join(", ") || "Sin material")}</small></div>`;
}
...
function renderPlanningActivities() {
  ...
  const unmatched = state.planningUnmatched.length
    ? `<div class="planning-unmatched"><strong>Atención:</strong> Mina reportó lugares que no aparecen en el plan: ${state.planningUnmatched.map(item => esc(item.lugar)).join(", ")}.</div>`
    : "";
  $('planningActivities').innerHTML = unmatched + state.planningItems.map((item, index) => `
    <article class="planning-activity">
      <div class="planning-activity-header"><div><strong>${esc(item.lugar || item.idActividad || item.idPlan)}</strong><span>${esc(item.cuadrilla || "Sin cuadrilla")} · ${esc(item.idProyecto || "Sin proyecto")}</span></div><span>${esc(item.prioridad || "")}</span></div>
      <div class="planning-activity-body">
        ${operationSummaryMarkup(item.operacion)}
        <div class="planning-result-grid">
          ...
          <label>Disparos programados<input value="${number(item.disparosProgramados)}" readonly /></label>
...
function validatePlanningReport() {
  ...
  for (const item of state.planningItems) {
    ...
    if (places.has(placeKey)) { alert(`Hay más de una actividad programada para ${item.lugar}. Consolida los puebles y disparos en una sola fila por lugar y fecha.`); return false; }
    places.add(placeKey);
    ...
    const incumple = pReal < number(item.pueblesProgramados) || dReal < number(item.disparosProgramados);
