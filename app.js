...
let catalogRows = [];
const state = { work: [], movement: [], equipment: {}, mpReasons: [], planningItems: [], plant: [] };
...
async function loadPlanningActivities() {
  ...
  try {
    ...
    state.planningItems = (result.datos || []).map(plan => ({ ...plan, pueblesRealizados: "", disparosRealizados: "", comentariosReal: "", causas: [] }));
    $('planningStatus').textContent = `${state.planningItems.length} actividad(es)`;
...
function renderPlanningActivities() {
  ...
  $('planningActivities').innerHTML = state.planningItems.map((item, index) => `
